...
# Визуализация
g = DynamicGraphs("SDG: Аппроксимация функции", figsize=(10, 6))
...
for i in range(N):
    ...
    updata_formula = f"$a(x) = {w[0]:.2f} {w[1]:+.2f}x {w[2]:+.2f}x^2 {w[3]:+.3f}x^3$"
    g.update_graphs(coord_x, new_y, new_label=updata_formula)

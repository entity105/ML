...
def df(w, x, y):
    M = np.dot(w, x) * y
    return -(np.exp(-M) * x.T * y) / ((1 + np.exp(-M)) * np.log(2))     # => Вектор w = (w1, w2, w3, w4, w5)
...
for i in range(N):
    ...
    grad = np.zeros(len(x_train[0]))
    for j in range(k, k+batch_size):
        grad += df(w, x_train[j], y_train[j]) / batch_size
        Qk += loss(w, x_train[j], y_train[j]) / batch_size
    l1 = lm_l1 * np.sign(w)
    l1[0] = 0
    w = w - nt * (grad + l1)
    Qe = lm * Qk + (1-lm) * Qe
